...
# Generated by create-quality-automation (Pro/Enterprise feature)
set -e
...
if [[ $RISK_SCORE -ge 6 ]]; then
  echo "🔴 HIGH RISK - Full validation"
  echo "   • Unit + integration tests"
  npm test
elif [[ $RISK_SCORE -ge 3 ]]; then
  echo "🟡 MEDIUM RISK - Unit tests"
  npm run test:unit
elif [[ $RISK_SCORE -ge 2 || "$SPEED_BONUS" == "false" ]]; then
  echo "🟢 LOW RISK - Lint only"
  npm run lint && npm run format:check
else
...
echo ""
echo "💡 Tip: Run 'npm test' locally for full validation"
