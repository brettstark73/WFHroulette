...
import { isoWeekInfo, pickWFHDay, pickReason, formatDay, simpleHash } from '../src/util.js';
...
test('simpleHash produces consistent results', () => {
  assertEqual(simpleHash('test'), simpleHash('test'), 'Same input should produce same hash');
  assertEqual(typeof simpleHash('test'), 'number', 'Hash should return number');
});

test('simpleHash handles edge cases', () => {
  assertEqual(simpleHash(''), simpleHash(''), 'Empty string should be consistent');
  assertEqual(simpleHash(null), simpleHash(null), 'Null should be consistent');
  assertEqual(simpleHash(undefined), simpleHash(undefined), 'Undefined should be consistent');
});
...
test('pickWFHDay is deterministic', () => {
  ...
  assertEqual(result1.date.getTime(), result2.date.getTime(), 'Same seed/date should produce same result');
...
test('pickWFHDay produces different results for different seeds', () => {
  ...
  if (!different) {
    console.warn('  Warning: Different seeds produced same date (low probability but possible)');
  }
});
...
test('formatDay formats dates correctly', () => {
  ...
  assertEqual(formatted.includes('Thursday'), true, 'Should include weekday name');
});
...
test('formatDay handles different dates', () => {
  ...
  assertEqual(formatted1.includes('2025-01-01'), true, 'Should format first day of year');
  assertEqual(formatted2.includes('2025-12-31'), true, 'Should format last day of year');
  assertEqual(formatted1 !== formatted2, true, 'Different dates should format differently');
});
...
test('Full workflow integration', () => {
  ...
  assertEqual(testReasons.includes(reason), true, 'Reason should be from test array');
  assertEqual(typeof formatted, 'string', 'Formatted date should be string');
  assertEqual(formatted.length > 10, true, 'Formatted date should be meaningful length');
});
