    ...
    serverProcess.on('exit', code => {
      if (code !== 0) {
        ...
        if (
          ...
          reject(
            new Error(`Server exited with code ${code}: ${errorOutput.trim()}`)
          )
        }
      }
    })
  ...
  try {
    ...
    await test('HTML contains required elements', async () => {
      ...
      const requiredElements = [
        ...
        'Generate My Excuse', // Button text
      ]
    ...
    if (error.message.includes('PORT_BINDING_FAILED')) {
      console.log(
        '⚠️  Integration tests skipped: Port binding not allowed in this environment'
      )
      console.log(
        '   (Set SKIP_INTEGRATION_TESTS=true to suppress this warning)'
      )
